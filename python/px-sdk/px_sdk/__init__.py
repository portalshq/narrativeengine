...
_DEFAULT_PX_DIR = os.environ.get("PX_DIR") or os.path.expanduser("~/.px")


def _resolve_repo_path(repo_path: str | None) -> str:
    if repo_path is None:
        return _DEFAULT_PX_DIR
...
def parse_uri(uri: str) -> dict[str, Any]:
    ...
    Args:
        uri: A PX URI, e.g. ``"px://starwars/character/lukeskywalker"``.
...
def uri_identity(uri: str) -> str:
    ...
    Args:
        uri: A PX URI, possibly with a fragment.

    Returns:
        The identity URI as a string, e.g. ``"px://starwars/character/lukeskywalker"``.
    """
...
def parse_manifest(yaml_str: str) -> dict[str, Any]:
    ...
    Args:
        yaml_str: A YAML string representing a PX manifest.
...
def repo_init(repository: str, base_path: str | None = None) -> dict[str, Any]:
    ...
    Args:
        repository: Repository name.
        base_path: Base directory for repository repos (defaults to ``$PX_DIR`` / ``~/.px``).
...
def resolve(
    ...
    Args:
        uri: PX URI (e.g. ``"px://starwars/character/lukeskywalker"``).
        repo_path: Base directory for repositories (defaults to ``$PX_DIR`` / ``~/.px``).
...
def presign_representation(
    ...
    *,
    ...
    the ``px://`` prefix is optional. ``representation`` is the manifest key,
