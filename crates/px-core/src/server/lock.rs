...
//!
//! Ensures only one local Lore daemon is started per PX home directory
...
impl ProcessLock {
    /// Create a new process lock for the given PX home directory
    pub fn new(px_home: &Path) -> Self {
        let lock_file = px_home.join("lore").join("pid");
        Self {
            lock_file,
            acquired: false,
        }
