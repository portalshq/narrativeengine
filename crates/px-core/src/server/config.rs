...
//! and recommended persistent defaults for PX-managed deployments.
...
/// - Persistent stores under PX home directory
/// - Persistent certificates
/// - Recommended defaults for local development
pub fn generate_local_config(px_home: &Path) -> Result<ConfigFiles> {
    let lore_config_dir = px_home.join("lore").join("config");
    ...
    let config = generate_config_toml(px_home);
...
fn generate_config_toml(px_home: &Path) -> String {
    let immutable_path = px_home.join("lore").join("store").join("immutable");
    let mutable_path = px_home.join("lore").join("store").join("mutable");
    let _cert_path = px_home.join("lore").join("certs").join("cert.pem");
    let _key_path = px_home.join("lore").join("certs").join("key.pem");
    ...
    format!(
        r#"
# =============================================================================
# Lore Server Configuration (Generated by PX SDK)
# =============================================================================
# This configuration is automatically generated by the PX SDK.
...
mod tests {
    ...
    fn test_generate_local_config() {
        ...
        let px_home = temp_dir.path();

        let files = generate_local_config(px_home).unwrap();
        ...
        let files2 = generate_local_config(px_home).unwrap();
    ...
    fn test_config_toml_content() {
        ...
        assert_eq!(
            ...
            px_home.join("lore").join("store").join("mutable").to_str()
        );
        ...
        let windows_config = config.replace(
            &toml_quote(&px_home.join("lore").join("store").join("immutable")),
