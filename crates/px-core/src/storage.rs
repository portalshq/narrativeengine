//! # Storage Engine — PX Asset Ingestion & Content-Addressed Storage
//!
...
//! based on the `PX_STORAGE_BACKEND` environment variable:
//!
//! | Value   | Backend                        | Required env vars               |
//! |---------|--------------------------------|---------------------------------|
//! | `local` | Local filesystem               | `PX_DIR` (default: `~/.px`)   |
//! | `s3`    | S3-compatible object store     | `PX_S3_BUCKET`, AWS creds      |
//!
//! ## Gotchas Handled
//!
...
//! * **Ignore management** — The local initialiser ensures `.px-assets/` is present
//!   in `<PX_DIR>/.gitignore` so raw binaries never enter the storage graph.
...
pub enum StorageBackend {
    /// Local filesystem, writing to `<PX_DIR>/.px-assets/`.
    Local,
...
pub struct StorageConfig {
    ...
    /// Subdirectory / prefix for asset blobs (`.px-assets`).
...
impl StorageEngine {
    ...
    /// |------------------------|------------|-----------------------------------------------|
    /// | `PX_STORAGE_BACKEND`  | both       | `local` (default) or `s3`                     |
    /// | `PX_DIR`              | local      | Base directory (default: `~/.px`)            |
    /// | `PX_S3_BUCKET`        | s3         | S3 bucket name (required)                     |
    ...
    pub fn from_env() -> StorageResult<Self> {
        let backend_str =
            std::env::var("PX_STORAGE_BACKEND").unwrap_or_else(|_| "local".to_string());
    ...
    fn init_local() -> StorageResult<Self> {
        let raw = std::env::var("PX_DIR").unwrap_or_else(|_| "~/.px".to_string());
        let base_dir = resolve_path(&raw);

        // Ensure the assets directory exists before any write.
        let assets_dir = base_dir.join(".px-assets");
        ...
        Ok(Self {
            ...
            config: StorageConfig {
                ...
                base_dir,
                assets_prefix: ".px-assets".to_string(),
                bucket: String::new(),
            },
        })
    }

    /// Ensure `<PX_DIR>/.gitignore` exists and contains `.px-assets/`.
    fn ensure_gitignore(base_dir: &Path) -> StorageResult<()> {
        let gitignore_path = base_dir.join(".gitignore");
        const ASSETS_ENTRY: &str = ".px-assets/";
        ...
        if !gitignore_path.exists() {
            ...
            info!(
                path = %gitignore_path.display(),
                "created .gitignore with .px-assets/ entry"
            );
            return Ok(());
        }
        ...
        let already_present = content.lines().any(|line| {
            ...
            trimmed == ASSETS_ENTRY || trimmed.trim_end_matches('/') == ".px-assets"
        });
        ...
        if !already_present {
            ...
            std::fs::write(&gitignore_path, updated).map_err(|e| {
                error!(
                    path = %gitignore_path.display(),
                    error = %e,
                    "failed to append .px-assets/ to .gitignore"
                );
                StorageError::Io(e)
            })?;
            info!(
                path = %gitignore_path.display(),
                "appended .px-assets/ entry to .gitignore"
            );
        } else {
            debug!(
                path = %gitignore_path.display(),
                ".gitignore already contains .px-assets/ entry"
            );
        }
    ...
    fn init_s3() -> StorageResult<Self> {
        let bucket = std::env::var("PX_S3_BUCKET").map_err(|_| {
            error!("PX_S3_BUCKET is required for S3 storage backend");
            StorageError::MissingEnvVar("PX_S3_BUCKET".to_string())
        })?;
    ...
    ///    prefix (`.px-assets/` for both backends).
    ///
    /// # Arguments
    ///
...
mod tests {
    ...
    fn test_resolve_path_expands_tilde() {
        let resolved = resolve_path("~/px-test-storage");
    ...
    fn test_ensure_gitignore_creates_when_missing() {
        ...
        assert!(
            content.contains(".px-assets/"),
            "expected .px-assets/ in .gitignore, got: {content:?}"
        );
    }
    ...
    fn test_ensure_gitignore_appends_when_entry_missing() {
        ...
        assert!(
            content.contains(".px-assets/"),
            "expected .px-assets/ to be appended, got: {content:?}"
        );
        assert!(
    ...
    fn test_build_store_path_local() {
        ...
        assert!(
            path_str.contains(".px-assets/abc123.png"),
            "expected path containing '.px-assets/abc123.png', got: {path_str}"
        );
    }
    ...
    fn test_build_store_path_s3() {
        ...
        let path = engine.build_store_path("abc123.png").unwrap();
        assert_eq!(path.to_string(), ".px-assets/abc123.png");
    }
    ...
    fn test_build_store_path_local_from_absolute_path() {
        ...
        let path_str = path.to_string();
        assert!(
            path_str.contains(".px-assets/abc.png"),
            "expected path to contain '.px-assets/abc.png', got: {path_str}"
        );
        // The path should not be a URL-style S3 key.
        assert!(
            !path_str.starts_with(".px-assets/"),
    ...
    fn test_storage_error_messages() {
        ...
        let err = StorageError::MissingEnvVar("PX_S3_BUCKET".to_string());
        assert_eq!(
            err.to_string(),
            "required environment variable `PX_S3_BUCKET` is not set"
        );
    ...
    async fn test_ingest_media_is_idempotent() {
        ...
        let hex = &hash1[7..];
        let path = StorePath::from(format!(".px-assets/{hex}.png"));
    ...
    async fn test_ingest_media_unknown_format() {
        ...
        let hex = &hash[7..];
        let path = StorePath::from(format!(".px-assets/{hex}.weird-format-123"));
        let meta = engine.store.head(&path).await.unwrap();
    ...
    async fn test_ingest_media_large_blob() {
        ...
        let hex = &hash[7..];
        let path = StorePath::from(format!(".px-assets/{hex}.bin"));
        let meta = engine.store.head(&path).await.unwrap();
    ...
    async fn test_local_ingest_writes_file_to_disk() {
        ...
        // The file should exist at <tmp>/.px-assets/<hex>.txt
        let hex = &hash[7..];
        let file_path = tmp.path().join(".px-assets").join(format!("{hex}.txt"));
        assert!(
            file_path.exists(),
    ...
    async fn test_local_ingest_idempotent_skips_write() {
        ...
        let hex = &hash1[7..];
        let dir = tmp.path().join(".px-assets");
    ...
    async fn test_local_ingest_multiple_files() {
        ...
        let assets_dir = tmp.path().join(".px-assets");
    ...
    async fn test_local_ingest_resolves_content_by_hash() {
        ...
        // returned hash is a valid PX content address.
        let tmp = tempfile::TempDir::new().unwrap();
        let engine = local_fs_engine(tmp.path());
        ...
        // Parse as a PX ContentHash — validates format & hex.
    ...
    async fn test_local_ingest_subdirectory_created() {
        ...
        assert!(file_path.exists(), "file should exist in .px-assets");
    }
    ...
    async fn test_ingest_media_with_nonexistent_base_dir_still_works() {
        ...
        // (except .px-assets which we create), it should work.
        let tmp = tempfile::TempDir::new().unwrap();
        let base = tmp.path().join("nested").join("path");

        // Create the .px-assets dir (simulating what init_local does).
        let assets = base.join(".px-assets");
        std::fs::create_dir_all(&assets).unwrap();
    ...
    async fn test_ingest_media_multiple_formats_same_content() {
        ...
        let hex = &hash_png[7..];
        let path_png = StorePath::from(format!(".px-assets/{hex}.png"));
        let path_jpg = StorePath::from(format!(".px-assets/{hex}.jpg"));
    ...
    async fn test_ingest_media_forces_unique_formats_in_store() {
        ...
        let hex = &hash_a[7..];
        let path_mp4 = StorePath::from(format!(".px-assets/{hex}.mp4"));
        let path_wav = StorePath::from(format!(".px-assets/{hex}.wav"));
