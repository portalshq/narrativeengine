//! PX URI parser and builder.
//!
//! The PX URI scheme identifies narrative resources:
//!
//! ```text
//! px://toystory/character/woody#appearances.audienceVotes
//! ───┬── ───┬──── ────┬──── ──────┬────── ─────────────┬───────────
//!  scheme repository  entity_type entity_id          fragment (query)
//! ```
//!
//! **Key design decisions:**
//! - Version/branch/tag are NEVER encoded in the URI path. They are orthogonal
//!   selectors passed alongside the URI (mirrors VCS, OCI, package managers).
//! - Fragment (`#`) carries the query path for subtree extraction.
//! - Entity type is any non-empty string — fully dynamic and user-defined.

use std::fmt;
use std::str::FromStr;

use serde::{Deserialize, Serialize};

use crate::error::PxError;
use crate::types::EntityType;

/// The `px://` URI scheme constant.
pub const PX_SCHEME: &str = "px://";
const LEGACY_NAP_SCHEME: &str = "nap://";

/// Returns whether an input has a URI scheme accepted by the PX reader.
///
/// This is deliberately the only public compatibility boundary for legacy
/// addresses. Callers should parse with [`PxUri`] and serialize the result to
/// obtain the canonical `px://` form.
pub fn has_supported_scheme(input: &str) -> bool {
    let input = input.trim();
    input.starts_with(PX_SCHEME) || input.starts_with(LEGACY_NAP_SCHEME)
}

/// A parsed PX URI representing a narrative resource identity.
///
/// # Examples
///
/// ```
/// use px_core::uri::PxUri;
///
/// let uri: PxUri = "px://toystory/character/woody#references.appears_in"
///     .parse()
///     .unwrap();
///
/// assert_eq!(uri.repository, "toystory");
/// assert_eq!(uri.entity_type.as_str(), "character");
/// assert_eq!(uri.entity_id, "woody");
/// assert_eq!(uri.fragment.as_deref(), Some("references.appears_in"));
/// ```
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct PxUri {
    /// The repository name (directory under base_dir). e.g., `"toystory"`, `"pokemon"`.
    pub repository: String,

    /// The kind of entity being addressed. Any non-empty string is valid.
    pub entity_type: EntityType,

    /// The entity's identifier (slug). e.g., `"woody"`, `"pikachu"`.
    pub entity_id: String,

    /// Optional fragment for subtree queries. e.g., `"appearances.audienceVotes"`.
    /// Populated from the `#` portion of the URI.
    pub fragment: Option<String>,
}

impl PxUri {
    /// Construct a new PX URI without a fragment.
    pub fn new(
        repository: impl Into<String>,
        entity_type: impl Into<EntityType>,
        entity_id: impl Into<String>,
    ) -> Self {
        Self {
            repository: repository.into(),
            entity_type: entity_type.into(),
            entity_id: entity_id.into(),
            fragment: None,
        }
    }

    /// Construct a PX URI with a fragment query path.
    pub fn with_fragment(
        repository: impl Into<String>,
        entity_type: impl Into<EntityType>,
        entity_id: impl Into<String>,
        fragment: impl Into<String>,
    ) -> Self {
        Self {
            repository: repository.into(),
            entity_type: entity_type.into(),
            entity_id: entity_id.into(),
            fragment: Some(fragment.into()),
        }
    }

    /// Returns the canonical URI string WITHOUT the fragment.
    /// This is the resource identity — fragments are query concerns.
    pub fn identity(&self) -> String {
        format!(
            "px://{}/{}/{}",
            self.repository, self.entity_type, self.entity_id
        )
    }

    /// Returns the relative filesystem path for this entity's manifest within
    /// a repository.
    ///
    /// e.g., `"character/woody.yaml"` or `"repository.yaml"` for repo metadata.
    pub fn manifest_path(&self) -> String {
        if self.entity_type.as_str() == "world" {
            "repository.yaml".to_string()
        } else {
            format!(
                "{}/{}.yaml",
                self.entity_type.directory_name(),
                self.entity_id
            )
        }
    }
}

impl fmt::Display for PxUri {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "px://{}/{}/{}",
            self.repository, self.entity_type, self.entity_id
        )?;
        if let Some(ref fragment) = self.fragment {
            write!(f, "#{fragment}")?;
        }
        Ok(())
    }
}

impl FromStr for PxUri {
    type Err = PxError;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let input = s.trim();

        if input.contains("://") && !has_supported_scheme(input) {
            return Err(PxError::InvalidUri {
                uri: input.to_string(),
                reason: "unsupported URI scheme; expected px://".to_string(),
            });
        }

        // ── Strip scheme (optional) ──────────────────────────────────────
        // Legacy NAP addresses are accepted only at this parsing boundary;
        // formatting always emits the canonical PX scheme.
        let without_scheme = input
            .strip_prefix(PX_SCHEME)
            .or_else(|| input.strip_prefix(LEGACY_NAP_SCHEME))
            .unwrap_or(input);

        // ── Split fragment ──────────────────────────────────────────────
        let (path_part, fragment) = match without_scheme.split_once('#') {
            Some((path, frag)) => {
                let frag_trimmed = frag.trim();
                if frag_trimmed.is_empty() {
                    (path, None)
                } else {
                    (path, Some(frag_trimmed.to_string()))
                }
            }
            None => (without_scheme, None),
        };

        // ── Parse path segments: repository / entity_type / entity_id ─────
        let segments: Vec<&str> = path_part.split('/').filter(|s| !s.is_empty()).collect();

        if segments.len() < 3 {
            return Err(PxError::InvalidUri {
                uri: input.to_string(),
                reason: format!(
                    "expected at least 3 path segments (repository/entity_type/entity_id), got {}",
                    segments.len()
                ),
            });
        }

        let repository = segments[0].to_string();
        let entity_type = EntityType::new(segments[1]);
        // Join remaining segments to support entity IDs with slashes (defensive)
        let entity_id = segments[2..].join("/");

        if repository.is_empty() {
            return Err(PxError::InvalidUri {
                uri: input.to_string(),
                reason: "repository name cannot be empty".to_string(),
            });
        }
        if entity_id.is_empty() {
            return Err(PxError::InvalidUri {
                uri: input.to_string(),
                reason: "entity ID cannot be empty".to_string(),
            });
        }

        Ok(PxUri {
            repository,
            entity_type,
            entity_id,
            fragment,
        })
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_parse_full_uri_with_fragment() {
        let uri: PxUri = "px://toystory/character/woody#appearances.audienceVotes"
            .parse()
            .unwrap();
        assert_eq!(uri.repository, "toystory");
        assert_eq!(uri.entity_type.as_str(), "character");
        assert_eq!(uri.entity_id, "woody");
        assert_eq!(uri.fragment.as_deref(), Some("appearances.audienceVotes"));
    }

    #[test]
    fn test_parse_uri_without_fragment() {
        let uri: PxUri = "px://toystory/location/pizza-planet".parse().unwrap();
        assert_eq!(uri.repository, "toystory");
        assert_eq!(uri.entity_type.as_str(), "location");
        assert_eq!(uri.entity_id, "pizza-planet");
        assert!(uri.fragment.is_none());
    }

    #[test]
    fn test_parse_legacy_uri_canonicalizes_to_px() {
        let uri: PxUri = "nap://toystory/character/woody".parse().unwrap();
        assert_eq!(uri.to_string(), "px://toystory/character/woody");
    }

    #[test]
    fn test_rejects_unsupported_scheme() {
        let error = "other://toystory/character/woody"
            .parse::<PxUri>()
            .unwrap_err();
        assert!(matches!(error, PxError::InvalidUri { .. }));
    }

    #[test]
    fn test_parse_custom_entity_type() {
        let uri: PxUri = "px://lab/paper/cold-fusion-v2".parse().unwrap();
        assert_eq!(uri.repository, "lab");
        assert_eq!(uri.entity_type.as_str(), "paper");
        assert_eq!(uri.entity_id, "cold-fusion-v2");
    }

    #[test]
    fn test_parse_scene_uri() {
        let uri: PxUri = "px://toystory/scene/pizza-planet".parse().unwrap();
        assert_eq!(uri.entity_type.as_str(), "scene");
        assert_eq!(uri.entity_id, "pizza-planet");
    }

    #[test]
    fn test_parse_world_uri() {
        let uri: PxUri = "px://toystory/world/toystory".parse().unwrap();
        assert_eq!(uri.entity_type.as_str(), "world");
    }

    #[test]
    fn test_roundtrip_display_parse() {
        let original = PxUri::with_fragment(
            "toystory",
            EntityType::new("character"),
            "woody",
            "references.appears_in",
        );
        let displayed = original.to_string();
        let parsed: PxUri = displayed.parse().unwrap();
        assert_eq!(original, parsed);
    }

    #[test]
    fn test_identity_strips_fragment() {
        let uri = PxUri::with_fragment(
            "toystory",
            EntityType::new("character"),
            "woody",
            "appearances",
        );
        assert_eq!(uri.identity(), "px://toystory/character/woody");
    }

    #[test]
    fn test_manifest_path_character() {
        let uri = PxUri::new("toystory", EntityType::new("character"), "woody");
        assert_eq!(uri.manifest_path(), "character/woody.yaml");
    }

    #[test]
    fn test_manifest_path_world() {
        let uri = PxUri::new("toystory", EntityType::new("world"), "toystory");
        assert_eq!(uri.manifest_path(), "repository.yaml");
    }

    #[test]
    fn test_manifest_path_custom_type() {
        let uri = PxUri::new("lab", EntityType::new("paper"), "cold-fusion-v2");
        assert_eq!(uri.manifest_path(), "paper/cold-fusion-v2.yaml");
    }

    #[test]
    fn test_invalid_too_few_segments() {
        let result = "px://toystory/character".parse::<PxUri>();
        assert!(result.is_err());
    }

    #[test]
    fn test_optional_scheme() {
        let uri: PxUri = "toystory/character/woody#references.appears_in"
            .parse()
            .unwrap();
        assert_eq!(uri.repository, "toystory");
        assert_eq!(uri.entity_type.as_str(), "character");
        assert_eq!(uri.entity_id, "woody");
        assert_eq!(uri.fragment.as_deref(), Some("references.appears_in"));
    }

    #[test]
    fn test_bare_path_no_fragment() {
        let uri: PxUri = "toystory/location/pizza-planet".parse().unwrap();
        assert_eq!(uri.repository, "toystory");
        assert_eq!(uri.entity_type.as_str(), "location");
        assert_eq!(uri.entity_id, "pizza-planet");
        assert!(uri.fragment.is_none());
    }

    #[test]
    fn test_bare_path_too_few_segments() {
        let result = "toystory/character".parse::<PxUri>();
        assert!(result.is_err());
    }
}
