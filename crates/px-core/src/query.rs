//! Subtree query engine for PX manifests.
//!
...
//! - CLI queries: `px resolve px://toystory/character/woody#references.appears_in`
...
impl ManifestQuery {
    ...
    /// ```
    /// use serde_yaml::Value;
    /// use px_core::query::ManifestQuery;
    ///
    /// let yaml = "
    /// name: Woody
    /// properties:
    ...
    /// ```
    pub fn query(root: &Value, path: &str, manifest_id: &str) -> Result<Value, PxError> {
        if path.is_empty() {
            return Err(PxError::InvalidQueryPath(
        ...
        for (depth, segment) in segments.iter().enumerate() {
            ...
            current = match current {
                ...
                Value::Sequence(seq) => {
                    ...
                    let index: usize = segment.parse().map_err(|_| PxError::QueryPathNotFound {
                        path: path.to_string(),
                        manifest_id: manifest_id.to_string(),
                    })?;
                    seq.get(index).ok_or_else(|| PxError::QueryPathNotFound {
                        path: path.to_string(),
                        manifest_id: manifest_id.to_string(),
                    })?
                }
                _ => {
                    return Err(PxError::QueryPathNotFound {
                        path: path.to_string(),
                        manifest_id: manifest_id.to_string(),
                    });
                }
    ...
    pub fn list_keys(root: &Value, path: &str, manifest_id: &str) -> Result<Vec<String>, PxError> {
...
references:
  appears_in:
    ...
    - "px://toystory/scene/claw-machine"
  relationships:
    - target: "px://toystory/character/slinky"
      type: parent
"#;
    ...
    fn test_query_array_index() {
        ...
        assert_eq!(
            result,
            Value::String("px://toystory/scene/pizza-planet".to_string())
        );
    }
