//! JSON Schema generation for PX core types.
//!
//! These schemas describe the PX data model (manifest, commit, etc.)
...
//! `px_get_schema` tool so that agents can introspect the data model
...
pub fn manifest_schema() -> Value {
    serde_json::json!({
        ...
        "properties": {
            "id": {
                "type": "string",
                "pattern": "^px://[a-zA-Z0-9_-]+/[a-zA-Z0-9_-]+/[a-zA-Z0-9_-]+$",
                "description": concat!(
                    "Canonical PX URI. ",
                ...
                "properties": {
                    ...
                    "derived_from": {
                        ...
                        "description": concat!(
                            ...
                            "e.g., px://toystory/character/woody/v1"
                        )
                    },
                    "created_at": {
                        "type": "string",
...
/// Returns the JSON Schema for a PX Commit.
pub fn commit_schema() -> Value {
    serde_json::json!({
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "px://schema/commit.json",
        "title": "PX Commit",
        "description": concat!(
            "A PX commit records a point-in-time snapshot of a manifest ",
...
mod tests {
    ...
    fn test_schemas_serialize_to_valid_json() {
        ...
        let parsed: Value = serde_json::from_str(&json).unwrap();
        assert_eq!(parsed.get("title").unwrap().as_str().unwrap(), "PX Commit");
    }
}
