//! BLAKE3 content addressing for PX resources.
//!
...
impl ContentHash {
    ...
    pub fn parse(s: &str) -> Result<Self, PxError> {
        ...
        hex::decode(hex_part)
            .map_err(|e| PxError::Other(format!("invalid hex in content hash: {e}")))?;
    ...
    pub fn verify(&self, data: &[u8]) -> Result<(), PxError> {
        let actual = Self::from_bytes(data);
        if *self != actual {
            return Err(PxError::ContentHashMismatch {
