...
impl LoreGrpcClient {
    ...
    pub async fn get_branch_by_name(&self, name: &str) -> Result<Branch, PxError> {
        ...
        response.into_inner().branch.ok_or_else(|| {
            PxError::GrpcError(format!("BranchGet({name}) returned empty branch record"))
        })
    }

    pub async fn list_branches(&self) -> Result<Vec<Branch>, PxError> {
        let mut client = self.make_client();
    ...
    ) -> Result<Vec<proto_gen::lore::model::v1::RevisionItem>, PxError> {
        let mut client = self.make_client();
        Ok(client
    ...
    async fn read_storage_fragment(
        &self,
        address: proto_gen::lore::model::v1::Address,
    ) -> Result<(proto_gen::lore::model::v1::Fragment, Vec<u8>), PxError> {
    ...
    pub async fn get_repository_by_name(
        &self,
        name: &str,
    ) -> Result<proto_gen::lore::model::v1::Repository, PxError> {
        ...
        client
            ...
            .repository
            .ok_or_else(|| {
                PxError::GrpcError(format!("RepositoryGet({name}) returned no repository"))
            })
    }

    /// Return names of repositories visible to the current identity.
    pub async fn list_repositories(&self) -> Result<Vec<String>, PxError> {
        let mut client = self.make_repository_client();
        let mut stream = client
    ...
    pub async fn read_file_at_revision(
        ...
        let address = address.ok_or_else(|| PxError::ManifestNotFound(path.clone()))?;
    ...
    pub async fn read_file_at_signature(
        ...
        let address = address.ok_or(PxError::ManifestNotFound(path))?;
        let bytes = self.read_storage_content(address).await?;
    ...
    pub async fn list_paths_at_revision(
        ...
        let mut stream = match response {
            ...
            // implementation detail to PX users.
            Err(status)
    ...
    pub async fn file_address_at_revision(
        ...
        identifier: proto_gen::lore::model::v1::RevisionIdentifier,
        path: String,
    ) -> Result<(proto_gen::lore::model::v1::Address, Vec<u8>), PxError> {
        let mut tree = self.make_thin_client();
        let mut stream = tree
            .revision_tree(Self::revision_tree_for_file(
                RevisionTreeQuery::Identifier(identifier),
        ...
        address
            ...
            .ok_or(PxError::ManifestNotFound(path))
    }

    pub async fn revision_info_at_signature(
        &self,
        signature: Vec<u8>,
    ) -> Result<proto_gen::lore::thin_client::v1::Revision, PxError> {
        ...
        client
            ...
            .into_inner()
            .revision
            .ok_or_else(|| PxError::GrpcError("RevisionInfo returned no revision".to_string()))
    }
}
...
fn storage_protocol_error(message: impl Into<String>) -> PxError {
    PxError::GrpcError(format!(
        "StorageGet (invalid Lore fragment): {}",
        message.into()
    ))
}

fn decode_fragment_references(payload: &[u8]) -> Result<Vec<FragmentReference>, PxError> {
...
/// |----------|----------|---------|-------------|
/// | `PX_LORE_GRPC_ENDPOINT` | Yes | — | gRPC endpoint URL |
/// | `PX_LORE_GRPC_TOKEN` | No | — | JWT bearer token |
/// | `PX_LORE_GRPC_RID` | No | — | Repository ID (hex-encoded binary) |
/// | `PX_LORE_GRPC_INSECURE` | No | `0` | Skip TLS verification when `1` |
#[derive(Default)]
pub struct Builder {
...
impl Builder {
    ...
    pub fn build(self) -> Result<LoreGrpcClient, PxError> {
        let endpoint_str = self.endpoint.ok_or_else(|| {
            PxError::GrpcError(
                "gRPC endpoint is required — set via .endpoint() or PX_LORE_GRPC_ENDPOINT"
                    .to_string(),
            )
        })?;
        ...
        let channel = Endpoint::from_shared(effective_url)
            .map_err(|e| {
                PxError::GrpcError(format!("invalid gRPC endpoint '{endpoint_str}': {e}"))
            })?
            .http2_keep_alive_interval(Duration::from_secs(30))
            .keep_alive_timeout(Duration::from_secs(20))
            .user_agent(concat!("px-core/", env!("CARGO_PKG_VERSION")))
            .map_err(|e| PxError::GrpcError(format!("user-agent configuration error: {e}")))?
            .connect_lazy();
    ...
    ///
    /// Returns `Ok(None)` when `PX_LORE_GRPC_ENDPOINT` is not set
    /// (allowing the caller to skip gRPC integration gracefully).
    pub fn from_env() -> Result<Option<LoreGrpcClient>, PxError> {
        let endpoint = match std::env::var("PX_LORE_GRPC_ENDPOINT") {
            Ok(v) => v,
            Err(_) => return Ok(None),
        };

        let token = std::env::var("PX_LORE_GRPC_TOKEN").ok();
        let insecure = std::env::var("PX_LORE_GRPC_INSECURE")
            .ok()
            .is_some_and(|v| v == "1" || v == "true" || v == "yes");

        let repository_id_bytes = std::env::var("PX_LORE_GRPC_RID")
            .ok()
            .map(|hex| {
                hex::decode(&hex).map_err(|e| {
                    PxError::GrpcError(format!("invalid PX_LORE_GRPC_RID hex '{hex}': {e}"))
                })
            })
            .transpose()?
            .unwrap_or_default();
...
/// * The closure return type is `Result<T, PxError>` so that error
///   propagation through the thread join is straightforward.
///
/// [`VcsBackend`]: crate::vcs::VcsBackend
pub fn block_on_grpc<F, T>(f: F) -> Result<T, PxError>
where
    F: Future<Output = Result<T, PxError>> + Send + 'static,
    T: Send + 'static,
{
    ...
    thread::Builder::new()
        ...
        .map_err(|panic_payload| {
            PxError::GrpcError(format!("gRPC thread panicked: {panic_payload:?}"))
        })?
}
...
/// Map a [`tonic::Status`] to a structured [`PxError`].
fn map_grpc_status(context: &str, status: tonic::Status) -> PxError {
    let code = status.code();
    let message = status.message();
    match code {
        tonic::Code::NotFound => PxError::RefNotFound(format!("{context}: {message}")),
