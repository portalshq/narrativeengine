# PX Integration Tests

This directory contains integration test suites for the PX CLI.
...
- Resolve manifest URIs using px resolver
...
cargo test -p portalshq-px-cli --test local_lore_suite --features lore-e2e -- --test-threads=1
```

### 2. Portals Cloud Lore Server Suite (`cloud_lore_suite.rs`)

Tests px functionality against the Portals Cloud lore server.
...
export PX_LORE_URL_BASE="grpcs://lore.portals.works"
export PX_WORKSPACE_ID="your-workspace-id"
...
- `px_cmd()`: Returns a configured Command instance for the px binary
...
2. Initialize px with appropriate provider
...
2. Check the server URL: `echo $PX_LORE_URL_BASE`
...
1. Verify environment variables are set:
   ```bash
   echo $PX_LORE_URL_BASE
   echo $PX_WORKSPACE_ID
...
Increase timeout in `px_cmd()` helper function if tests are timing out due to slow network or server response times.
  ...
  local-tests:
    ...
    steps:
      ...
      - name: Run Local Integration Tests
        run: cargo test -p portalshq-px-cli --test local_lore_suite --features lore-e2e -- --test-threads=1
  ...
  cloud-tests:
    ...
    steps:
      ...
      - name: Run Cloud Integration Tests
        env:
          PX_LORE_URL_BASE: ${{ secrets.PX_LORE_URL_BASE }}
          PX_WORKSPACE_ID: ${{ secrets.PX_WORKSPACE_ID }}
          PORTALS_CLOUD_API_KEY: ${{ secrets.PORTALS_CLOUD_API_KEY }}
        run: cargo test -p portalshq-px-cli --test cloud_lore_suite --features lore-e2e -- --test-threads=1
```
