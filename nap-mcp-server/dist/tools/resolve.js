 ...
 */
import { z } from "zod";
import { parseNapUri, resolveManifest, getHistory, NapApiError, NapNotFoundError, } from "../services/api.js";
import { CONFIG } from "../constants.js";
// ── Zod Schemas ───────────────────────────────────────────────────────────
const NapUrnField = z
    .string()
    .describe("NAP URI, e.g. 'nap://starwars/character/lukeskywalker'. " +
...
const ResolveManifestInputSchema = z
    ...
    path: z
        .string()
        .optional()
        .describe("Subtree query path. Overrides URI fragment. " +
 ...
 */
function truncateLargeArrays(value, depth = 0) {
    if (depth > 10)
        return value;
...
export function registerResolveTools(server) {
    // ── resolve_manifest ───────────────────────────────────────────────
    server.registerTool("nap_resolve_manifest", {
...
Examples:
  ...
  - "Get the canon version of Tatooine" → uri="nap://starwars/location/tatooine", branch="canon"`,
        ...
        try {
            ...
            const truncated = text.length > CONFIG.characterLimit
    ...
    const QueryPropertyInputSchema = z
        ...
        path: z
            .string()
            .describe("Dot-notation path to the property. " +
            "Examples: 'properties.species', 'representations.reference_image.hash', " +
        ...
        .strict();
    server.registerTool("nap_query_property", {
...
Examples:
  ...
  - "What's the hash of the reference image?" → path="representations.reference_image.hash"`,
    ...
    const GetHistoryInputSchema = z
        ...
        .strict();
    server.registerTool("nap_get_history", {
...
function formatError(err) {
    ...
    if (err instanceof NapApiError) {
        const suggestions = [];
        if (err.status === 400) {
            suggestions.push("NAP URIs look like: nap://starwars/character/lukeskywalker");
            suggestions.push("Valid entity types: character, location, scene, prop, world");
        }
        if (err.status === 502 || err.status === 408) {
            suggestions.push("Make sure nap-server is running: `cargo run -p nap-server`");
        }
