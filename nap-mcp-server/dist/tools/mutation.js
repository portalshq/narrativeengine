 ...
 */
import { z } from "zod";
import { parseNapUri, resolveManifest, commitChanges, revertCommit, NapApiError, } from "../services/api.js";
// ── Tool: nap_set_property ────────────────────────────────────────────────
const SetPropertyInputSchema = z
    .object({
    uri: z
        .string()
        .describe("NAP URI of the entity to modify. " +
        "e.g., 'nap://starwars/character/lukeskywalker'"),
    key: z
        .string()
        .min(1)
        .describe("Property key (dot-notation for nested paths). " +
        "Examples: 'species', 'homeworld', 'affiliation', " +
        "'provenance.model', 'references.appears_in'."),
    value: z
        .string()
        .min(1)
        .describe("Property value as a string.  For structured data (arrays, objects), " +
...
export function registerMutationTools(server) {
    server.registerTool("nap_set_property", {
        title: "Set Property",
...
Cross-reference examples:
  ...
  - Prop → owner: use a nap://character URI`,
            ...
            try {
                ...
                return {
                    ...
                    content: [
                        {
                            type: "text",
                            text: `Error: Entity '${params.uri}' not found. ` +
    ...
    const CommitManifestInputSchema = z
        ...
        uri: z
            .string()
            .describe("NAP URI of the entity to update. " +
            "e.g., 'nap://starwars/character/lukeskywalker'"),
        properties: z
            .record(z.unknown())
            .describe("Map of property keys to values. " +
        ...
        .strict();
    server.registerTool("nap_commit_manifest", {
  ...
  - Record full provenance:
    ...
    const RevertInputSchema = z
        ...
        commit: z
            .string()
            .min(1)
            .describe("Commit hash to revert.  Must be a full SHA-1 hash (40 chars) " +
            "resolvable in the universe's Git history. " +
            "Use nap_get_history to find commit hashes."),
        author: z
            .string()
            .default("nap-agent")
            .describe("Author identifier for the revert commit " +
            "(e.g., 'agent@studio.com')."),
    })
        .strict();
    server.registerTool("nap_revert_commit", {
        title: "Revert Commit",
        ...
        if (err.status === 404) {
            return {
                ...
                content: [
                    {
                        type: "text",
                        text: `Error: Resource not found. The URI may be incorrect, or the universe/entity doesn't exist.\n` +
