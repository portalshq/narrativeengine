 ...
 */
import { z } from "zod";
import { listUniverses, listEntities, searchEntities, healthCheck, NapApiError, } from "../services/api.js";
import { CONFIG, ENTITY_TYPES } from "../constants.js";
export function registerDiscoveryTools(server) {
    // ── nap_list_universes ──────────────────────────────────────────────
    server.registerTool("nap_list_universes", {
  ...
  - "Show me all available worlds" → call with no args`,
            ...
            if (universes.length === 0) {
                return {
                    content: [
                        {
                            type: "text",
                            text: "No universes found. Make sure nap-server is pointing at a directory " +
    ...
    const ListEntitiesInputSchema = z
        ...
        entity_type: z
            .enum(ENTITY_TYPES)
            .optional()
            .describe("Optional filter: 'character', 'location', 'scene', 'prop', 'world'. " +
            "If omitted, all entity types are returned."),
    })
        .strict();
    server.registerTool("nap_list_entities", {
        title: "List Entities",
...
Examples:
  ...
  - "What locations exist in Toy Story?" → universe="toystory", entity_type="location"`,
        ...
        try {
            ...
            if (allEmpty) {
                return {
                    content: [
                        {
                            type: "text",
                            text: `No entities found in universe '${params.universe}'. ` +
                                (params.entity_type
            ...
            // Add summary counts
            const output = { universe: params.universe };
    ...
    const SearchEntitiesInputSchema = z
        ...
        query: z
            .string()
            .min(1)
            .describe("Search term. Matches against entity names and IDs (case-insensitive substring match). " +
        ...
        .strict();
    server.registerTool("nap_search_entities", {
...
Examples:
  ...
  - "Find Vader across all types" → universe="starwars", query="vader"`,
        ...
        try {
            const results = await searchEntities(params.universe, params.query, params.entity_type);
            if (results.length === 0) {
                return {
                    content: [
                        {
                            type: "text",
                            text: `No entities matching '${params.query}' found in '${params.universe}'. ` +
    ...
    // ── nap_health_check ────────────────────────────────────────────────
    server.registerTool("nap_health_check", {
        title: "Health Check",
