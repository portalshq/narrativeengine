...
### Added
- **Unified `px configure` command**: Single entry point for backend configuration, replacing `px choose` + `px backend` (both now hidden/deprecated with `set`, `--remote-url`/`--endpoint` aliases preserved). Supports positional provider (`local`, `portals-cloud`, `remote`), `--reset`, bootstrap flags, and `config` alias.
- **`px completions` command**: Generate shell completions via `clap_complete`; `completions/` (bash, zsh, fish) is now generated by `px-docgen` from the same Clap tree as the CLI reference.
- **Expanded `px doctor` diagnostics** with broader server/provider health checks.
- **VCS lore stale-remote detection**: push/pull/sync now detect when a checkout tracks a different Lore server than the configured provider and report an actionable error (re-clone or `px doctor`) instead of an opaque transport failure.
- **Self-hosted Lore auth guide** (`docs/authored/self-hosted-lore-auth.md`) and **production presign requirements** (`docs/authored/production-presign-requirements.md`); installation and usage docs updated.

### Fixed
- `publish-all.sh` release bump now tolerates per-package version skew (Python/TS manifests lagging the Cargo workspace version) instead of aborting.
- Docgen prunes stale generated command pages for renamed/hidden commands so the reference cannot advertise an obsolete CLI surface.

### Changed
- Bumped all package versions to 0.8.24.
