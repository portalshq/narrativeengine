...
# already listening on localhost, provision one through PX in a temporary
...
if curl --fail --silent --show-error --max-time 2 "$LORE_HEALTH_URL" >/dev/null; then
  ...
  echo "Installing Lore through PX..."
  cargo run -p portalshq-px-cli -- install lore

  echo "Starting a temporary local Lore server..."
  # Mark ownership before starting so the exit trap can clean up even if PX
  # starts the daemon but reports a later initialization error.
  STARTED_SERVER=1
  cargo run -p portalshq-px-cli -- --base-dir "$SERVER_HOME" init "$BOOTSTRAP_REPOSITORY" --provider local

  if [ -f "$SERVER_HOME/lore/pid" ]; then
    SERVER_PID="$(tr -d '[:space:]' < "$SERVER_HOME/lore/pid")"
  fi
  if ! [[ "$SERVER_PID" =~ ^[0-9]+$ ]]; then
    echo "Error: PX started Lore but did not record a valid server PID" >&2
    exit 1
  fi
fi
